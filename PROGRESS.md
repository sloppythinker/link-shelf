...
- 結果: 既存テスト全45件パス（store 28 + dom 17）。ローカルプレビュー（port 8340/8341）で現行版と比較確認済み。push 済み（SWキャッシュを v8 に更新し、インストール済みPWAにも index.html の変更が届くようにした）
- 次のステップ: 実機スマホでフォルダメニュー・インポートモーダル・トースト削減の感触確認
